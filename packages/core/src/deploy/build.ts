...
export function generateProvidedPluginsNitroPluginSource(
  ...
  return `// AUTO-GENERATED by @agentnative-fork/core deploy build
 ...
 * `@agentnative-fork/core/server`. This is the middle layer of the three-layer
...
export function generateWorkerEntry(
  ...
  // @agentnative-fork/core default, if the workspace core provides one.
  ...
  for (let i = 0; i < edgeDefaultStems.length; i++) {
    ...
    if (workspaceCore && workspaceExportName) {
      ...
      // Fall back to the framework default from @agentnative-fork/core.
      const defaultExportName = DEFAULT_PLUGIN_REGISTRY[stem];
      if (!defaultExportName) continue;
      pluginImports.push(
        `import { ${defaultExportName} as ${varName} } from "@agentnative-fork/core/server";`,
      );
    }
    pluginCalls.push(`  if (typeof ${varName} === "function") {
    await ${varName}(nitroApp);
        ...
        for (const { symlinkPath, pkgName } of candidates) {
          ...
          try {
            ...
            const stubPkgJson = {
              ...
              version: "0.0.0-stub",
              description:
                "Empty stub created by @agentnative-fork/core deploy build to satisfy nitro's file tracer on platforms where this optional dep is not installed.",
            };
            fs.writeFileSync(
...
async function buildWithNitro() {
  ...
  const agentsBundleModuleSource = () => {
    ...
    return `// AUTO-GENERATED by @agentnative-fork/core deploy build (Nitro virtual)
