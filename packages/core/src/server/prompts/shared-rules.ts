...
export function sharedRule8(examples?: PromptExamples): string {
  ...
  }, or any external data source. If the user asks about a table that is NOT in the app schema (e.g. \`dbt_analytics.*\`, \`dbt_mart.*\`, or any fully-qualified \`project.dataset.table\`), use the appropriate template action instead — ${warehouseExample}${providerExamples ? `${providerExamples} for their respective providers, ` : ""}etc. When the user names an external provider, that named provider action wins; do not substitute a warehouse tool like BigQuery unless the user explicitly asks for the warehouse copy. **Never use \`db-query\` for external data — it will fail.** When \`provider-api-catalog\`, \`provider-api-docs\`, and \`provider-api-request\` are available, first-class provider actions are shortcuts, not limits: inspect the provider API and call the exact endpoint/filter/body/pagination the user's question requires instead of weakening the answer to fit a narrow action. For broad searches, cross-source joins, corpus-wide counts/classification, or absence claims, fetch every relevant page or an explicitly bounded cohort, stage or save large responses, and reduce them with \`query-staged-dataset\` or \`run-code\` rather than loading raw records into chat. State coverage, filters, row counts, failed pages, and gaps; never infer "none found" from sampled, truncated, default-limited, or aborted results. For extensions, use \`get-extension\` when you already have an id from \`<current-screen>\` or \`<current-url>\`; otherwise use \`list-extensions\`, \`update-extension\`, \`hide-extension\`, and \`delete-extension\`. Do not query the legacy \`tools\` table directly.`;
}
