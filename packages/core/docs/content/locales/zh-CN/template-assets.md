...
- **嵌入为选择器。**其他应用程序可以 iframe `/picker` 并侦听来自 `@agentnative-fork/embedding` 的 `chooseAsset` 事件，将资产转变为博客编辑器、网站构建器、幻灯片和自定义应用程序的资产选择器/生成器。选择器还会为现有的仅图像主机发出旧版 `chooseImage` 别名。
...
1. **安装一次。**这会添加技能说明并一起注册托管的 MCP 连接器：
   ...
   ```bash
   npx @agentnative-fork/core@latest skills 添加资产 # 别名：图像生成
   ```
