...
| 连接铸造JWT          | `npx @agentnative-fork/core@latest connect` / Connect 页面铸造一个每用户、可撤销的 JWT                   |
...
刷新令牌仅存储为哈希值，并在每次刷新时轮换。默认情况下，`npx @agentnative-fork/core@latest connect` 为 Claude 代码客户端写入仅 URL 的 OAuth 条目；保留连接页面、`npx @agentnative-fork/core@latest connect --token <token>` 和静态承载配置以用于本地 stdio 代理、旧客户端和紧急/调试流程。
