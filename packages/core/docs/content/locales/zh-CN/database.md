...
每个模板通过从 `@agentnative-fork/core/db` 调用 `createGetDb(schema)` 来创建一个惰性的单例 Drizzle 客户端。规范位置是 `server/db/index.ts`：
...
从此模板本地路径导入 `getDb` - 路由中的 `../../server/db/index.js`，actions 中的 `../server/db/index.js` - 而不是直接从 `@agentnative-fork/core` 导入。核心导出返回一个通用的无类型实例；模板的 `getDb()` 包含您的架构类型。请参阅 [Server](/docs/server#request-context) 了解 actions 和自定义路由如何导入它。
...
使用 `@agentnative-fork/core/db/schema` 中的框架架构助手：
...
切勿直接从 `drizzle-orm/sqlite-core` 或 `drizzle-orm/pg-core` 导入。始终使用 `@agentnative-fork/core/db/schema`。
