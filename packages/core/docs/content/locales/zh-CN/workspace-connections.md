...
从`@agentnative-fork/core/connections`导入目录：
...
- **提供商目录：**提供商 ID、标签、功能、推荐的模板用途以及来自 `@agentnative-fork/core/connections` 的所需凭证密钥名称。
- **工作区摘要：**来自 `@agentnative-fork/core/workspace-connections` 的连接计数、活动/授权计数、授权状态、凭证引用名称和非秘密帐户标签。
...
1. **提供者元数据：**在 `@agentnative-fork/core/connections` 中添加或重用提供者。这是稳定 ID、显示标签、功能列表、推荐模板用途和凭证密钥名称。
2. **工作区连接：** Dispatch 或其他工作区设置界面通过 `@agentnative-fork/core/workspace-connections` 存储连接帐户的安全元数据、状态、范围、`credentialRefs` 和应用程序授权。
