...
工作流程编写完成后，运行`npx @agentnative-fork/core@latest recap setup`进行配置
GitHub Actions 秘密/变量（如果可能）和 `npx @agentnative-fork/core@latest recap doctor`
验证存储库是否已准备就绪。
...
已终止`，不要继续重试。使用
`npx -y @agentnative-fork/core@latest reconnect https://plan.agent-native.com --client codex`对于 Codex，或在支持 OAuth 的主机中重新运行`/mcp` → **身份验证**。开始一个
