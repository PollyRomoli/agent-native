...
- **与数据库无关。** 使用 `@agentnative-fork/core/db/schema` 写入模式并使用 Drizzle 的可移植查询 DSL 进行读/写，因此相同的代码可以在任何支持的提供程序上运行。仅将原始 SQL 用于附加迁移或一次性维护，保持参数化且与方言无关。参见[Database](/docs/database)。
