...
Não está pronto para criar um modelo inteiro? Você também pode experimentar o agente nativo adicionando uma **habilidade** a um agente de codificação que você já usa — instale a habilidade Planos com `npx @agentnative-fork/core@latest skills add visual-plan`. Veja o [Skills Guide](/docs/skills-guide#app-backed-skills).
