...
يوفر `@agentnative-fork/core` أدوات ربط وأدوات مساعدة React لجانب المتصفح لتطبيقات الوكيل الأصلية.

يتم تصدير هؤلاء العملاء/React API من `@agentnative-fork/core` و`@agentnative-fork/core/client`. قم باستيرادها من `@agentnative-fork/core/client` (إدخال المتصفح) للحصول على الوضوح والتجميع الصحيح، نظرًا لأن جذر `@agentnative-fork/core` المجرد يتحول إلى إنشاء Node افتراضيًا.
...
واستخدم الجسر المضيف فقط للسلوك المملوك للمضيف. `@agentnative-fork/core/client`
