...
- **لوحة الوكيل المضمنة** — يتم تضمينها داخل كل تطبيق من `@agentnative-fork/core`.
...
`@agentnative-fork/core` - لا توجد حزمة منفصلة لتثبيتها - وهي نفسها
مكون في التطوير والإنتاج.

- تم التصدير بتنسيق `AgentPanel` من `@agentnative-fork/core/client`، مع
...
من `@agentnative-fork/core/client`) مدعومًا بخادم PTY. يمكنك تشغيل
...
تأتي هذه العناصر مع `@agentnative-fork/core` وهي ما يستخدمه تطبيقك للتحدث مع
