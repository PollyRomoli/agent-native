...
عند اختبار Claude من خلال ngrok، استخدم إصدار الإنتاج (`npx @agentnative-fork/core@latest build` ثم `npx @agentnative-fork/core@latest start`) أو معاينة/إنتاج منشور URL. يعمل مسار الزرع أحادي الإطار Claude مع قطع أصول الإنتاج؛ يمكن حماية وحدات تطوير Vite الخام مثل `/app/root.tsx` من خلال مصادقة التطبيق وفشل عمليات الاستيراد الديناميكية من أصل مورد Claude.
...
يمكن للمسارات المضمنة استخدام `updateMcpAppModelContext()` و`openMcpAppHostLink()` و`requestMcpAppDisplayMode()` و`getMcpAppHostContext()` و`useMcpAppHostContext()` من `@agentnative-fork/core/client`. يستخدم `sendToAgentChat()` نفس المسار من عمليات تضمين التطبيق الكامل للمطالبات المقدمة تلقائيًا.
