...
يقوم كل قالب بإنشاء عميل Drizzle كسول ومفرد عن طريق استدعاء `createGetDb(schema)` من `@agentnative-fork/core/db`. الموقع الأساسي هو `server/db/index.ts`:
...
قم باستيراد `getDb` من هذا المسار المحلي للقالب — `../../server/db/index.js` في المسارات، و`../server/db/index.js` في actions — بدلاً من `@agentnative-fork/core` مباشرةً. يقوم التصدير الأساسي بإرجاع مثيل عام غير مكتوب؛ يحمل `getDb()` الخاص بالقالب أنواع المخططات الخاصة بك. راجع [Server](/docs/server#request-context) لمعرفة كيفية استيراد كل من actions والمسارات المخصصة له.
...
استخدم مساعدات مخطط إطار العمل من `@agentnative-fork/core/db/schema`:
...
لا تقم مطلقًا بالاستيراد من `drizzle-orm/sqlite-core` أو `drizzle-orm/pg-core` مباشرةً. استخدم دائمًا `@agentnative-fork/core/db/schema`.
