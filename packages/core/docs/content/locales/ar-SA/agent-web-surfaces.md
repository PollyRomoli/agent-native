...
- تدقيق CLI (`npx @agentnative-fork/core@latest audit-agent-web`) يتحقق من كل ما سبق.
...
تقوم أدوات إطار العمل المساعدة في `@agentnative-fork/core/agent-web` بإنشاء الملفات الشائعة من قائمة صفحات واحدة:
...
يمكن لتطبيقات Vite استخدام `createAgentWebVitePlugin` من `@agentnative-fork/core/vite` لكتابة هذه الملفات إلى `public` أو `dist` أو `dist/client` أو `dist/server/public` أو `build/client` أثناء إنشاءات الإنتاج.
