...
**تغيير البيانات أو الإعدادات.** توجد مخططات جداول التتبع والهياكل ذات الصلة في `templates/mail/server/db/`. تمر إعدادات القراءة والكتابة عبر `readSetting` / `writeSetting` من `@agentnative-fork/core/settings`. تستخدم حالة التطبيق (التنقل، والمسودات، والأوامر المفردة) `readAppState` / `writeAppState` من `@agentnative-fork/core/application-state`.
