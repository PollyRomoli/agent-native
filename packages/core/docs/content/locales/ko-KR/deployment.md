...
공급자 빌드 명령을 구성할 때 `--build-only`와 동일한 명령을 사용하십시오. Vercel은 `npx @agentnative-fork/core@latest deploy --preset vercel --build-only`를 실행해야 합니다. 이 명령은 `.vercel/output`를 직접 작성하므로 작업공간 라우팅에는 `vercel.json`가 필요하지 않습니다.
...
앱별 독립 배포는 계속 지원됩니다. 독립 실행형 스캐폴드처럼 `cd apps/<name> && npx @agentnative-fork/core@latest build`만 지원됩니다.

## 작동 방식 {#how-it-works}

`npx @agentnative-fork/core@latest build`를 실행하면 Nitro는 클라이언트 SPA와 서버 API를 모두 `.output/`에 빌드합니다.
...
| `APP_BASE_PATH`             | 접두사(예: `/mail`) 아래에 앱을 마운트합니다. `npx @agentnative-fork/core@latest deploy`에 의해 자동으로 설정됩니다. 독립 실행형의 경우 설정하지 않은 상태로 둡니다. |
