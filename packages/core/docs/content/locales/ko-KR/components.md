...
기본 `@agentnative-fork/core` 패키지에서 UI 구성 요소를 가져오지 마십시오. 사용
`@agentnative-fork/core/client` 또는 집중된 `@agentnative-fork/core/client/*` 하위 경로
...
| ------------------------------------ | ----------------------------------------------- | ---------------------------------------------------------------------------------- |
| `<AgentSidebar>`                     | `@agentnative-fork/core/client` 또는 `/client/chat` | 앱 주변에 완전한 사이드바가 필요합니다.                                            |
| `<AgentToggleButton>`                | `@agentnative-fork/core/client` 또는 `/client/chat` | 사이드바에 헤더 버튼을 직접 렌더링합니다.                                          |
| `<AgentPanel>`                       | `@agentnative-fork/core/client` 또는 `/client/chat` | 자신만의 레이아웃, 경로, 대화 상자 또는 측면 열에 전체 패널이 포함되기를 원합니다. |
| `<AgentChatSurface>`                 | `@agentnative-fork/core/client` 또는 `/client/chat` | 사이드바 래퍼 없이 패널 또는 페이지 모드에서 채팅을 원합니다.                      |
| `<AssistantChat>`                    | `@agentnative-fork/core/client` 또는 `/client/chat` | 표준 대화 및 작성기 런타임을 유지하면서 주변 크롬을 소유하고 싶습니다.             |
| `<MultiTabAssistantChat>`            | `@agentnative-fork/core/client` 또는 `/client/chat` | `AgentPanel` 크롬이 없는 프레임워크의 스레드 탭을 원합니다.                        |
| `createHttpAgentChatRuntime()`       | `@agentnative-fork/core/client` 또는 `/client/chat` | 정규화된 채팅 이벤트를 스트리밍하는 BYO 에이전트 엔드포인트가 있습니다.            |
| `createOpenAIAgentsChatRuntime()`    | `@agentnative-fork/core/client` 또는 `/client/chat` | OpenAI 상담원 SDK 스트림이 있고 그 주변에 표준 채팅 UI가 있기를 원합니다.          |
| `createOpenAIResponsesChatRuntime()` | `@agentnative-fork/core/client` 또는 `/client/chat` | OpenAI 응답 이벤트 스트림이 있고 이를 채팅 UI로 정규화하려고 합니다.               |
| `createAgUiChatRuntime()`            | `@agentnative-fork/core/client` 또는 `/client/chat` | AG-UI 이벤트 스트림이 있고 이를 채팅 UI로 정규화하려고 합니다.                     |
| `createClaudeAgentChatRuntime()`     | `@agentnative-fork/core/client` 또는 `/client/chat` | Claude 에이전트 SDK 스트림이 있고 이를 채팅 UI로 정규화하려고 합니다.              |
| `createVercelAiChatRuntime()`        | `@agentnative-fork/core/client` 또는 `/client/chat` | Vercel AI SDK 스트림이 있고 이를 채팅 UI로 정규화하려고 합니다.                    |
| `createAgentChatRuntimeAdapter()`    | `@agentnative-fork/core/client` 또는 `/client/chat` | `AgentChatRuntime`를 어시스턴트 UI에 직접 적용해야 합니다.                         |
| `createAgentChatAdapter()`           | `@agentnative-fork/core/client` 또는 `/client/chat` | 저수준 보조 UI 어댑터로 내장된 Agent-Native SSE 전송이 필요합니다.                 |
| `useChatThreads()`                   | `@agentnative-fork/core/client` 또는 `/client/chat` | 맞춤 스레드 목록, 기록 선택기 또는 범위 지정 채팅 UI가 필요합니다.                 |
| `sendToAgentChat()`                  | `@agentnative-fork/core/client` 또는 `/client/chat` | 제품 작업은 상담원 채팅에서 직접 수행해야 합니다.                                  |
...
동일한 채팅이 필요한 경우 `@agentnative-fork/core/client/composer`를 사용하세요
...
대본 스타일 렌더링에 `@agentnative-fork/core/client/conversation` 사용
전체 에이전트 런타임 외부
...
`@agentnative-fork/core/client/chat` 및 루트 클라이언트 항목에서. 참조
...
Liveblocks 스타일의 존재감을 위해 `@agentnative-fork/core/client/collab`를 사용하고
협업 문서 후크.
...
하위 수준 `@agentnative-fork/core/collab` 에이전트 존재 도우미:
...
공유 마크다운 편집기가 필요할 때 `@agentnative-fork/core/client/editor`를 사용하세요
...
동일한 내용을 노출하고 싶을 때 `@agentnative-fork/core/client/resources`를 사용하세요
...
| 지역        | APIs                                                  | 가져오기 경로                             |
| ----------- | ----------------------------------------------------- | ----------------------------------------- |
| 공유        | `<ShareButton>`, `<ShareDialog>`, `<VisibilityBadge>` | `@agentnative-fork/core/client/sharing`       |
| 알림        | `<NotificationsBell>`                                 | `@agentnative-fork/core/client/notifications` |
| 진행상황    | `<RunsTray>`, 진행 후크 및 유형                       | `@agentnative-fork/core/client/progress`      |
| 온보딩      | `useOnboarding()`, 온보딩 패널 후크                   | `@agentnative-fork/core/client/onboarding`    |
| 관측성      | `<ObservabilityDashboard>`, `<ThumbsFeedback>`        | `@agentnative-fork/core/client/observability` |
| 자원        | `<ResourcesPanel>`, `<ResourceTree>`, 리소스 후크     | `@agentnative-fork/core/client/resources`     |
| 리치 에디터 | `<SharedRichEditor>`, 슬래시 명령, 블록 노드 후크     | `@agentnative-fork/core/client/editor`        |

## 일회성 텍스트 완성 {#one-off-text-completion}

원시 텍스트 입력/텍스트 출력이 정말로 필요한 경우 서버측에 유지하고 사용하세요
`@agentnative-fork/core/server`의 `completeText()`. 사용자 대상 사용법을
