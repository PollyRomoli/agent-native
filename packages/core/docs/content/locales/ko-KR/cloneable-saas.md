...
다중 선택 선택 도구가 제공됩니다. 하나의 앱(독립 실행형) 또는 여러 개(작업 공간 - 앱 공유 인증, 브랜드, 에이전트 구성 및 데이터베이스)를 선택합니다. 선택한 각 템플릿은 필요한 모든 파일과 함께 `apps/<name>/`에 스캐폴딩됩니다. 템플릿 UI 대신 작업 전용 앱의 경우 `npx @agentnative-fork/core@latest create my-agent --headless`를 사용하세요.

`.env`(주로 `ANTHROPIC_API_KEY` 및 `DATABASE_URL`), `pnpm install`, `pnpm dev`를 입력하면 작동합니다. "TODO: 로그인 구현" 없음, 자리 표시자 경로 없음.

배포 대상: 모든 Nitro 호환 호스트(Node, Cloudflare, Netlify, Vercel, Deno, Lambda, Bun) 및 모든 Drizzle 호환 SQL 데이터베이스(SQLite, Postgres, Turso, D1, Supabase, Neon). 작업 공간의 경우 `npx @agentnative-fork/core@latest deploy`는 모든 앱을 한 번에 빌드하여 단일 원본으로 제공합니다. [Deployment](/docs/deployment)를 참조하세요.
