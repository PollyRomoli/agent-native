...
**데이터 또는 설정 변경.** 추적 테이블 및 관련 구조에 대한 스키마는 `templates/mail/server/db/`에 있습니다. 설정 읽기 및 쓰기는 `@agentnative-fork/core/settings`에서 `readSetting` / `writeSetting`를 통해 이루어집니다. 애플리케이션 상태(탐색, 초안, 원샷 명령)는 `@agentnative-fork/core/application-state`의 `readAppState` / `writeAppState`를 사용합니다.
