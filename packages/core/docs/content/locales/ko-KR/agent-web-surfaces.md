...
- 위의 사항을 모두 확인하는 감사 CLI(`npx @agentnative-fork/core@latest audit-agent-web`)
...
`@agentnative-fork/core/agent-web`의 프레임워크 유틸리티는 하나의 페이지 목록에서 공통 파일을 생성합니다.
...
Vite 앱은 프로덕션 빌드 중에 `@agentnative-fork/core/vite`의 `createAgentWebVitePlugin`를 사용하여 해당 파일을 `public`, `dist`, `dist/client`, `dist/server/public` 또는 `build/client`에 쓸 수 있습니다.
