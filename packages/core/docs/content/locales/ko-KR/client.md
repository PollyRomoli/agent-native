...
`@agentnative-fork/core`는 에이전트 네이티브 앱의 브라우저 측을 위한 React 후크 및 유틸리티를 제공합니다.

이 클라이언트/React API는 `@agentnative-fork/core` 및 `@agentnative-fork/core/client` 모두에서 내보내집니다. 기본적으로 `@agentnative-fork/core` 루트는 노드 빌드로 확인되므로 명확성과 올바른 번들링을 위해 `@agentnative-fork/core/client`(브라우저 항목)에서 가져옵니다.
...
호스트 소유 동작에만 호스트 브리지를 사용합니다. `@agentnative-fork/core/client`
