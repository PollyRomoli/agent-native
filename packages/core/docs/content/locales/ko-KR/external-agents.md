...
이전에 이전 베어러 토큰 흐름을 통해 Claude 코드를 연결한 경우 동일한 `npx @agentnative-fork/core@latest connect ... --client claude-code` 명령을 다시 실행하세요. CLI는 레거시 `Authorization` 헤더를 URL 전용 OAuth 항목으로 대체하고 `/mcp`에서 재인증하라고 알려줍니다.
...
`npx @agentnative-fork/core@latest connect https://assets.agent-native.com`와 같은 커넥터가 필요합니다.
...
이는 `npx @agentnative-fork/core@latest connect https://dispatch.agent-native.com --client claude-code`가 귀하를 위해 작성하는 것과 동일한 URL 전용 항목입니다. 그런 다음 Claude 코드에서 `/mcp`를 실행하고 **인증**을 선택합니다. 클라이언트는 MCP 서버의 `401 WWW-Authenticate` 챌린지에서 인증을 검색하고, `/.well-known/oauth-protected-resource` 및 `/.well-known/oauth-authorization-server`를 가져오고, 공개 OAuth 클라이언트를 동적으로 등록하고, 앱의 인증 페이지를 열고, 결과 토큰을 안전하게 저장합니다. ChatGPT 개발자 모드 커넥터는 동일한 서버 URL를 사용합니다.
...
클라이언트가 명시적인 범위를 요청하지 않으면 앱은 세 가지 범위를 모두 허용하므로 커넥터는 브라우저에서 승인된 Connect 흐름처럼 작동합니다. 바로 붙여넣을 수 있는 구성 블록이 필요한 로컬 개발, 대체 호스트 및 클라이언트에 대해 베어러 토큰 연결 페이지와 `npx @agentnative-fork/core@latest connect --token <token>` 대체를 유지하세요.
...
- **토큰당** — JWT에 `catalog_scope: "full"` 클레임을 포함하는 `--full-catalog`로 발행됩니다. 후속 요청은 해당 토큰에 대한 압축 필터를 우회합니다.
  ...
  ````배시
  npx @agentnative-fork/core@latest 연결 https://plan.agent-native.com --client codex --full-catalog
  ```
...
앱을 로컬에서 실행한 후(`pnpm dev` / `npx @agentnative-fork/core@latest dev`) 다음 명령 하나로 로컬 에이전트를 지정하세요.
...
항목은 기본적으로 실행 중인 로컬 앱의 `/_agent-native/mcp`에 대한 **얇은 stdio 프록시**인 `npx @agentnative-fork/core@latest mcp serve --app <id>`를 실행합니다. 따라서 라이브 액션 레지스트리, HMR 및 올바른 딥 링크는 단일 정보 소스로 유지됩니다. 대신 프로세스 내에서 레지스트리를 빌드하려면 `--standalone`를 전달하세요. `npx @agentnative-fork/core@latest mcp install`가 호스팅된 원본(작업 공간 `.env`의 로컬 호스트가 아닌 `APP_URL` / `BETTER_AUTH_URL` / `AGENT_NATIVE_MCP_URL`)을 감지하면 stdio 항목 대신 `Bearer` JWT를 사용하여 `<origin>/_agent-native/mcp`를 가리키는 `http` 클라이언트 항목을 작성합니다.

동반 하위 명령:

| 명령                                                       | 무엇을 하는가                                                     |
| ---------------------------------------------------------- | ----------------------------------------------------------------- |
| `npx @agentnative-fork/core@latest mcp serve [--app <id>]`     | MCP stdio 전송을 실행합니다(클라이언트 구성이 생성됨).            |
| `npx @agentnative-fork/core@latest mcp install --client <c>`   | 토큰을 프로비저닝하고 클라이언트의 MCP 구성을 작성합니다(멱등성). |
| `npx @agentnative-fork/core@latest mcp uninstall --client <c>` | 클라이언트 구성(멱등성)에서 명명된 MCP 항목을 제거합니다.         |
| `npx @agentnative-fork/core@latest mcp status`                 | 해결된 MCP URL/포트, 토큰 상태 및 클라이언트별 항목을 표시합니다. |
| `npx @agentnative-fork/core@latest mcp token [--rotate]`       | 작업 공간 `.env`에서 로컬 `ACCESS_TOKEN`를 인쇄(또는 회전)합니다. |
...
- `npx @agentnative-fork/core@latest connect https://dispatch.agent-native.com`를 사용하여 자신의 에이전트를 Dispatch에 연결하세요. 하나의 격리된 앱을 원하는 경우에만 직접 앱 URL를 사용하세요.
