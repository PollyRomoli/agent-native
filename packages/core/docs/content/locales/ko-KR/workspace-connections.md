...
`@agentnative-fork/core/connections`에서 카탈로그 가져오기:
...
- **공급자 카탈로그:** `@agentnative-fork/core/connections`의 공급자 ID, 레이블, 기능, 권장 템플릿 사용 및 필수 자격 증명 키 이름.
- **작업 공간 요약:** `@agentnative-fork/core/workspace-connections`의 연결 수, 활성/허가된 수, 부여 상태, 자격 증명 참조 이름, 비밀이 아닌 계정 라벨.
...
1. **공급자 메타데이터:** `@agentnative-fork/core/connections`에서 공급자를 추가하거나 재사용합니다. 이는 안정적인 ID, 표시 레이블, 기능 목록, 권장 템플릿 사용 및 자격 증명 키 이름입니다.
2. **작업 공간 연결:** 디스패치 또는 다른 작업 공간 설정 화면은 연결된 계정의 안전한 메타데이터, 상태, 범위, `credentialRefs` 및 `@agentnative-fork/core/workspace-connections`를 통한 앱 부여를 저장합니다.
