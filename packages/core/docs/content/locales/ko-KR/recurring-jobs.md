...
반복 작업을 `@agentnative-fork/scheduling`와 혼동하지 마십시오:

- **반복 작업(이 페이지)** — cron-scheduled _prompts_ 에이전트가 백그라운드에서 실행됩니다. 프레임워크 수준. 작업 공간에 거주합니다. 모든 에이전트 기반 앱에서 실행됩니다.
- **`@agentnative-fork/scheduling`** — 달력/예약 기능(이벤트 유형, 이용 가능 기간, 예약) 구축을 위한 재사용 가능한 도메인 패키지입니다. `calendar` 템플릿과 맞춤형 일정 관리 화면을 강화합니다.
