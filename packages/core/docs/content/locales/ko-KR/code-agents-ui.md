...
Agent-Native 코드는 코딩 세션, 슬래시 명령, 마이그레이션, 감사, 기록, 실행 제어 및 후속 작업을 위한 로컬 Claude 코드/Codex 스타일 작업 공간인 Agent-Native 코딩 표면입니다. 기본 `npx @agentnative-fork/core@latest` 명령으로 이 작업 공간이 열립니다. `npx @agentnative-fork/core@latest code`는 동일한 경험에 대한 명시적 하위 명령입니다.

3개의 레이어가 있습니다:

- **CLI**: `npx @agentnative-fork/core@latest` 및 `npx @agentnative-fork/core@latest code` 실행을 시작, 재개, 검사 및 중지합니다.
- **데스크톱**: 왼쪽 사이드바 코드 탭은 동일한 실행 모델을 사용하면서 기본 터미널 실행, 앱 웹뷰 및 데스크톱 딥 링크를 추가합니다.
- **공유 UI**: `@agentnative-fork/code-agents-ui`는 재사용 가능한 React 표면을 렌더링합니다.
...
각각은 동일한 내용을 노출하는 `@agentnative-fork/core/code-agents`를 호출합니다
...
명시적인 네임스페이스를 원할 경우 `npx @agentnative-fork/core@latest code`를 사용하세요. 슬래시 내장
...
코드 읽기를 통해 `@agentnative-fork/core/code-agents`에서 백그라운드 실행 내보내기
...
`@agentnative-fork/core/client/composer`에서 내보낸 `TiptapComposer` 스택
...
로컬 소스 경로는 읽기 전용입니다. 생성된 출력은 소스 트리 외부에 있어야 합니다. `--emit <dir>`를 사용하여 휴대용 마이그레이션 서류(`AGENTS.md`, `MIGRATION_PLAYBOOK.md`, 평가 및 사용 가능한 경우 `ir.json` 인벤토리)를 작성하고 내부 실행 표면을 여는 대신 다른 코딩 에이전트에 전달합니다. `/migrate`는 프레임워크의 일반 자격 증명 시스템을 재사용하므로 마이그레이션 관련 키 저장소가 없습니다. `@agentnative-fork/migrate` 패키지는 맞춤형 워크플로우를 위해 재사용 가능한 엔진(`createMigrationRun`, `discoverMigration`, `planMigration`, 소스/타겟 어댑터)을 공개합니다.
