...
`@agentnative-fork/core/data-widgets`; 클라이언트 코드는
`@agentnative-fork/core/client/chat` 또는 `@agentnative-fork/core/client`.
...
모든 커넥터는 `@agentnative-fork/core/client/chat`(및 루트)에서 내보내집니다.
`@agentnative-fork/core/client` 항목). 에이전트가 작동할 때 일반 HTTP 런타임을 사용하세요.
