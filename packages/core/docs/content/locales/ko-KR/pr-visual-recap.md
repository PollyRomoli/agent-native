...
형제를 포함한 `@agentnative-fork/core@latest`의 `visual-recap` 지침
...
| 비밀                | 목적                                                                                                                     |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------ |
| `PLAN_RECAP_TOKEN`  | `npx @agentnative-fork/core@latest connect`가 발행한 취소 가능한 토큰입니다. 요약 계획 게시 및 스크린샷 업로드를 승인합니다. |
...
**솔로: 개인 토큰은 여전히 작동합니다.** `npx @agentnative-fork/core@latest connect`로 발행하세요
Plans 앱에 반대합니다. 호스팅된 앱의 경우 로컬
`npx @agentnative-fork/core@latest recap setup`가 읽을 수 있는 게시 토큰 파일:
...
| `RECAP_CLI_VERSION`      | `latest`                        | 변수입니다. 워크플로가 설치하는 `@agentnative-fork/core` CLI 버전을 고정합니다. `1.5.0`. [Version pinning](#version-pinning-copy-variant)를 참조하세요. |
| `PLAN_RECAP_APP_URL`     | `https://plan.agent-native.com` | 비밀. 다른 출처에서 Plans 앱을 자체 호스팅하는 경우에만 해당됩니다.                                                                                 |

워크플로는 도우미 CLI(이 모노레포 내의 로컬 소스, 다른 곳에 게시된 `@agentnative-fork/core`)를 호출하는 방법을 자동으로 감지하므로 설정할 `RECAP_CLI` 변수가 없습니다.
...
기본적으로 복사 변형 워크플로는 런타임에 `@agentnative-fork/core@latest`를 설치하므로 모든 요약 실행이 자동으로 최신 CLI를 선택합니다. CI에 재현 가능한 도구가 필요한 경우 **`RECAP_CLI_VERSION`** 저장소 변수를 설정하여 설치된 버전을 고정하세요.
...
요약을 게시하기 전에 워크플로는 `npx @agentnative-fork/core@latest recap scan`를 실행하여 차이점에서 가능성 있는 비밀을 감지합니다. diff가 알려진 비밀 패턴과 일치하는 PR은 설명 주석과 함께 차단됩니다. 요약은 게시되지 않으며 diff 콘텐츠는 코딩 에이전트로 전송되지 않습니다.
...
기본 설치 프로그램은 전체 ~360줄 워크플로 YAML를 저장소에 복사합니다(**복사** 옵션). 이는 실행되는 모든 라인을 감사해야 하는 에어 갭 저장소 또는 저장소에 적합한 선택입니다. 단점은 버그 수정 및 개선 사항이 사용자에게 전달되지 않는다는 것입니다. 각 릴리스 후에 `npx @agentnative-fork/core@latest recap setup`를 수동으로 다시 실행해야 합니다.
...
이것은 `npx @agentnative-fork/core@latest recap setup --reusable`가 쓴 내용입니다(또는 수동으로 붙여넣을 수도 있습니다):
...
작성 후 평소처럼 `npx @agentnative-fork/core@latest recap doctor`를 실행하여 비밀이 구성되었는지 확인하세요.
...
`cli-version` 입력은 `@agentnative-fork/core` CLI 버전이 워크플로 내에서 실행되는 것을 제어합니다. 최신 릴리스를 추적하려면 `"latest"`에 남겨두거나 완전한 재현성을 위해 버전 문자열(예: `"1.5.0"`)에 고정하세요.
