...
실제 프레임워크 프리미티브인 `useAgentRouteState`(`@agentnative-fork/core/client`에서 내보낸)를 둘러싼 얇은 래퍼이기 때문에 짧게 유지됩니다. 두 가지 앱별 기능을 제공하면 프레임워크가 나머지 작업을 수행합니다.
...
브라우저 측 애플리케이션 상태 액세스를 위해 `@agentnative-fork/core/client`의 `setClientAppState`, `writeClientAppState`, `readClientAppState` 및 `deleteClientAppState`를 사용합니다. `useDbSync({ ignoreSource: TAB_ID })`와 페어링할 때 UI 쓰기에 `{ requestSource: TAB_ID }`를 전달합니다. 언로드 중 선택 정리와 같은 단기 쓰기에는 `{ keepalive: true }`를 전달합니다.
