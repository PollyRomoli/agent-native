...
| Connect-minted JWT      | `npx @agentnative-fork/core@latest connect` / 연결 페이지는 사용자별로 취소 가능한 JWT를 생성합니다                                    |
...
새로 고침 토큰은 해시로만 저장되며 새로 고칠 때마다 교체됩니다. `npx @agentnative-fork/core@latest connect`는 기본적으로 Claude 코드 클라이언트에 대해 이 URL 전용 OAuth 항목을 작성합니다. 연결 페이지, `npx @agentnative-fork/core@latest connect --token <token>`, 로컬 stdio 프록시, 이전 클라이언트 및 긴급/디버그 흐름을 위한 정적 베어러 구성을 유지합니다.
