...
- **선택기로 삽입.** 다른 앱은 `/picker`를 iframe하고 `@agentnative-fork/embedding`에서 `chooseAsset` 이벤트를 수신하여 자산을 블로그 편집자, 사이트 빌더, 슬라이드 데크 및 사용자 정의 앱을 위한 자산 선택기/생성기로 전환할 수 있습니다. 또한 선택기는 기존 이미지 전용 호스트에 대한 레거시 `chooseImage` 별칭을 내보냅니다.
...
1. **한 번 설치합니다.** 기술 지침이 추가되고 호스팅된 MCP 커넥터가 함께 등록됩니다.
   ...
   `````배시
   npx @agentnative-fork/core@latest skills 자산 # 별칭 추가: 이미지 생성
   ```
