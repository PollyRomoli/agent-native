...
Cada plantilla crea un cliente Drizzle único y diferido llamando a `createGetDb(schema)` desde `@agentnative-fork/core/db`. La ubicación canónica es `server/db/index.ts`:
...
Importe `getDb` desde esta ruta local de plantilla (`../../server/db/index.js` en rutas, `../server/db/index.js` en actions) en lugar de hacerlo directamente desde `@agentnative-fork/core`. La exportación principal devuelve una instancia genérica sin tipo; El `getDb()` de la plantilla lleva sus tipos de esquema. Consulte [Server](/docs/server#request-context) para saber cómo lo importan actions y las rutas personalizadas.
...
Utilice los ayudantes de esquema del marco de `@agentnative-fork/core/db/schema`:
...
Nunca importe desde `drizzle-orm/sqlite-core` o `drizzle-orm/pg-core` directamente. Utilice siempre `@agentnative-fork/core/db/schema`.
