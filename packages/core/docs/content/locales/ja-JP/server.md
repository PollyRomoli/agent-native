...
`getDb` は、`server/db/index.ts` の `createGetDb(schema)` 経由でアプリごとに作成されるため、カスタム ルートは `@agentnative-fork/core/db` からではなく、テンプレート (`../../db/index.js`) からインポートします。 [Database — Where the DB Client Lives](/docs/database#db-client)を参照してください。カスタム ルートではスコープ外の `db.select().from(ownableTable)` を実行しないでください。
