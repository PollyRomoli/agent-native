...
| ----------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| **埋め込みアプリ ピッカー**         | 完全な Agent-Native アプリを集中 iframe (アセット ピッカー、フォーム ビルダー、承認パネル) として起動します。                    | `@agentnative-fork/embedding`                |
| **`<AgentNative>` ホスト ブリッジ** | ページ コンテキストとクライアント actions を手動で接続するスタンドアロン サイドカー アプリまたはクロスオリジン iframe。          | `@agentnative-fork/core/client`              |
| **ポータブル拡張機能**              | SaaS がすでに拡張機能のストレージ/承認を所有している場合に、ホスト ユーザーがサンドボックス ミニアプリを構築できるようにします。 | `@agentnative-fork/core/client` 拡張スロット |

下位レベルの `@agentnative-fork/embedding` パッケージは以下を公開します:

| インポートパス                     | それが提供するもの                                                                                  |
| ---------------------------------- | --------------------------------------------------------------------------------------------------- |
| `@agentnative-fork/embedding`          | `EmbeddedApp` ピッカー コンポーネント、`getA2AUrl`、`getMcpUrl`、`sendMessage` (ストリーミング A2A) |
| `@agentnative-fork/embedding/react`    | React 固有のフックとコンポーネント                                                                  |
| `@agentnative-fork/embedding/bridge`   | `announceEmbeddedAppReady`、`sendEmbeddedAppMessage` — 埋め込みアプリ内で使用されます               |
| `@agentnative-fork/embedding/agent`    | エージェント エンドポイント ヘルパー                                                                |
| `@agentnative-fork/embedding/protocol` | プロトコルの種類                                                                                    |
...
ホスト製品が完全なバージョンを起動したい場合は、`@agentnative-fork/embedding` を使用します
