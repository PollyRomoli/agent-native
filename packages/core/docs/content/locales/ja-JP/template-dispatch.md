...
- **Vault スキーマ。** シークレット、許可、リクエスト、承認、監査ログ用の Drizzle テーブル。これらは `@agentnative-fork/dispatch` パッケージ (`packages/dispatch/src/db/schema.ts`) 内に存在し、`templates/dispatch/server/db/index.ts` 経由でテンプレートに再エクスポートされます。テンプレートローカルの `server/db/schema.ts` はありません。 Dispatch のランタイムは、テンプレート ソースではなくパッケージに同梱されています (`@agentnative-fork/dispatch` がシェル、サイドバー、組み込みページを所有しているという以下の注記と一致します)。
...
追加のタブとルートのみ。 `@agentnative-fork/dispatch` はシェルを所有し続けます。
