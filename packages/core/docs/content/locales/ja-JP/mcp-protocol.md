...
| コネクトミント JWT             | `npx @agentnative-fork/core@latest connect` / 接続ページはユーザーごとに取り消し可能な JWT を作成します                            |
...
リフレッシュ トークンはハッシュとしてのみ保存され、リフレッシュのたびにローテーションされます。 `npx @agentnative-fork/core@latest connect` は、デフォルトで、Claude コード クライアントに対してこの URL 専用の OAuth エントリを書き込みます。接続ページ、`npx @agentnative-fork/core@latest connect --token <token>`、ローカル stdio プロキシ、古いクライアント、緊急/デバッグ フロー用の静的ベアラー設定を保持します。
