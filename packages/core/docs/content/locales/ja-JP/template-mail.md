...
**データまたは設定の変更。** 追跡テーブルおよび関連構造のスキーマは、`templates/mail/server/db/` にあります。設定の読み取りと書き込みは、`@agentnative-fork/core/settings` から `readSetting` / `writeSetting` を経由します。アプリケーションの状態 (ナビゲーション、ドラフト、ワンショット コマンド) は、`@agentnative-fork/core/application-state` の `readAppState` / `writeAppState` を使用します。
