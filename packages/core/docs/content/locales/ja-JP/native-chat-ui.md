...
`@agentnative-fork/core/data-widgets`;クライアント コードは、
`@agentnative-fork/core/client/chat` または `@agentnative-fork/core/client`。
...
すべてのコネクタは `@agentnative-fork/core/client/chat` (およびルート) からエクスポートされます
`@agentnative-fork/core/client` エントリ)。エージェント
