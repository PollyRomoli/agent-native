...
- 上記のすべてをチェックする監査 CLI (`npx @agentnative-fork/core@latest audit-agent-web`)。
...
`@agentnative-fork/core/agent-web` のフレームワーク ユーティリティは、1 つのページ リストから共通ファイルを生成します。
...
Vite アプリは、実稼働ビルド中に `@agentnative-fork/core/vite` の `createAgentWebVitePlugin` を使用して、これらのファイルを `public`、`dist`、`dist/client`、`dist/server/public`、または `build/client` に書き込むことができます。
