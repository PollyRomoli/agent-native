...
定期的なジョブと `@agentnative-fork/scheduling` を混同しないでください:

- **定期的なジョブ (このページ)** — cron でスケジュールされた _プロンプト_ エージェントがバックグラウンドで実行されます。フレームワークレベル。ワークスペースに住んでいます。あらゆるエージェントネイティブ アプリ上で実行されます。
- **`@agentnative-fork/scheduling`** — カレンダー/予約機能 (イベント タイプ、空き状況ウィンドウ、予約) を構築するための再利用可能なドメイン パッケージ。 `calendar` テンプレートとカスタム スケジュール サーフェスを強化します。
