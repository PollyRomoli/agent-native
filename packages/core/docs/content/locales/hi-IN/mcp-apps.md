...
एम्बेडेड रूट `@agentnative-fork/core/client` से `updateMcpAppModelContext()`, `openMcpAppHostLink()`, `requestMcpAppDisplayMode()`, `getMcpAppHostContext()` और `useMcpAppHostContext()` का उपयोग कर सकते हैं। `sendToAgentChat()` स्वतः सबमिट किए गए संकेतों के लिए पूर्ण-ऐप एम्बेड से समान पथ का उपयोग करता है।
