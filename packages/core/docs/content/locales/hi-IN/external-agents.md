...
यदि आपने पहले Claude कोड को पुराने बियरर-टोकन प्रवाह के माध्यम से कनेक्ट किया है, तो बस वही `npx @agentnative-fork/core@latest connect ... --client claude-code` कमांड फिर से चलाएँ। CLI पुराने `Authorization` हेडर को केवल URL OAuth प्रविष्टि से बदल देता है और आपको `/mcp` से पुनः प्रमाणित करने के लिए कहता है।
...
`npx @agentnative-fork/core@latest connect https://assets.agent-native.com` जैसे कनेक्टर की आवश्यकता है।
...
- **प्रति टोकन** - `--full-catalog` के साथ टकसाल, जो JWT में `catalog_scope: "full"` दावा एम्बेड करता है। बाद के अनुरोध उस टोकन के लिए कॉम्पैक्ट फ़िल्टर को बायपास करते हैं:
  ...
  ```बैश
  npx @agentnative-fork/core@latest कनेक्ट https://plan.agent-native.com --क्लाइंट कोडेक्स --फुल-कैटलॉग
  ```
...
अपना ऐप स्थानीय रूप से चलाएं (`pnpm dev` / `npx @agentnative-fork/core@latest dev`), फिर एक आदेश के साथ उस पर एक स्थानीय एजेंट को इंगित करें:
...
प्रविष्टि `npx @agentnative-fork/core@latest mcp serve --app <id>` चलाती है, जो डिफ़ॉल्ट रूप से चल रहे स्थानीय ऐप के `/_agent-native/mcp` के लिए एक **पतली stdio प्रॉक्सी** है - इसलिए लाइव एक्शन रजिस्ट्री, HMR और सही डीप लिंक सत्य का एकल स्रोत बने रहते हैं। इसके बजाय प्रक्रिया में रजिस्ट्री बनाने के लिए `--standalone` पास करें। जब `npx @agentnative-fork/core@latest mcp install` एक होस्टेड मूल (कार्यस्थान `.env` में एक गैर-लोकलहोस्ट `APP_URL` / `BETTER_AUTH_URL` / `AGENT_NATIVE_MCP_URL`) का पता लगाता है, तो यह एक stdio प्रविष्टि के बजाय `Bearer` JWT के साथ `<origin>/_agent-native/mcp` की ओर इशारा करते हुए एक `http` क्लाइंट प्रविष्टि लिखता है।

सहयोगी उपकमांड:

| कमांड                                                      | यह क्या करता है                                                               |
| ---------------------------------------------------------- | ----------------------------------------------------------------------------- |
| `npx @agentnative-fork/core@latest mcp serve [--app <id>]`     | MCP stdio ट्रांसपोर्ट चलाएं (क्लाइंट कॉन्फिगरेशन क्या पैदा करता है)।          |
| `npx @agentnative-fork/core@latest mcp install --client <c>`   | एक टोकन प्रदान करें + क्लाइंट का MCP कॉन्फिगरेशन लिखें (इडेम्पोटेंट)।         |
| `npx @agentnative-fork/core@latest mcp uninstall --client <c>` | क्लाइंट के कॉन्फिगरेशन (इडेम्पोटेंट) से नामित MCP प्रविष्टि को हटा दें।       |
| `npx @agentnative-fork/core@latest mcp status`                 | समाधान किए गए MCP URL/पोर्ट, टोकन स्थिति और प्रति-ग्राहक प्रविष्टियां दिखाएं। |
| `npx @agentnative-fork/core@latest mcp token [--rotate]`       | कार्यस्थान `.env` में स्थानीय `ACCESS_TOKEN` को प्रिंट करें (या घुमाएँ)।      |
...
- `npx @agentnative-fork/core@latest connect https://dispatch.agent-native.com` के साथ प्रेषण के लिए अपने स्वयं के एजेंट को कनेक्ट करें; डायरेक्ट ऐप URL का उपयोग केवल तभी करें जब आपको एक अलग ऐप चाहिए।
